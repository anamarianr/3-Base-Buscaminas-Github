...
public class VentanaPrincipal {
	...
	public void mostrarNumMinasAlrededor(int i, int j) {
		...
		JLabel min = new JLabel(minas2,SwingConstants.CENTER);		
				...
				if(minas == 0) {
					...
					min.setBackground(correspondenciaColores[0]);
					
					
				}
				if(minas == 1) {
				
					min.setBackground(correspondenciaColores[1]);
					
				}
				if(minas == 2) {
					
					min.setBackground(correspondenciaColores[2]);
					
				}
				if(minas == 3) {
					
					min.setBackground(correspondenciaColores[3]);
				}
				if(minas >= 4) {
					
					min.setBackground(correspondenciaColores[4]);
	...
	public void mostrarFinJuego(boolean porExplosion) {
		...
		if(porExplosion) {
			...
			JOptionPane.showInputDialog(ventana, "Has perdido", 0);
